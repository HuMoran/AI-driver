...
| `/ai-driver:run-spec <文件>`    | 端到端执行 spec：**Phase 0 spec 审查** → 规划 → 实现 → 测试 → 提 PR。加 `--review-only` 只跑 Phase 0（草稿迭代，不切分支） |
...
| 命令                     | 建议会话设置                                       |
| ------------------------ | -------------------------------------------------- |
| `/ai-driver:run-spec`    | Opus + `xhigh` effort（多步规划 + TDD + 任务编排）。`--review-only` 同样推荐 Opus（对抗性读 spec） |
...
```markdown
constitution.md     — 项目规则（P1-P6、R-001 到 R-009）
...
`/ai-driver:run-spec <spec> --review-only` 让你在切分支前先对草稿 spec 做预检 — 与第 1 道门同一套 Layer 0 + subagent + Codex，Phase 0 跑完即退出。
