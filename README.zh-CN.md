...
| 命令 | 作用 | 输入 | 输出 |
|------|------|------|------|
| `/run-spec <文件>` | 执行 spec 全流程 | spec 文件路径 | PR + 实现日志 |
| `/review-pr [编号]` | 双盲审查 PR | PR 号（可选） | GitHub PR 评论 |
| `/fix-issues` | 批量修复 issue | --label, --limit | 每个 issue 一个 PR |
| `/run-tests` | 运行测试 | --type | 测试报告 |
| `/deploy <环境>` | 部署 | staging/production | 部署报告 |
