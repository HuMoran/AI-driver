...
If your project ships its own Claude Code plugin (like AI-driver itself does), the `.github/workflows/template-sync.yml` generated by `/ai-driver:init --with-ci` enforces byte-identity between repo files and their mirrored counterparts under `plugins/*/templates/…`. Any PR that edits only one side fails loudly with a `cp` command to fix it.

## Periodic health check

After a major AI-driver framework upgrade, or if the project starts behaving unexpectedly, run `/ai-driver:doctor`. It is read-only and reports drift (CLAUDE.md not importing AGENTS.md, stale constitution.md, legacy v0.1 filenames, plugin version skew) with copy-paste fix commands. Safe to run in CI too.
