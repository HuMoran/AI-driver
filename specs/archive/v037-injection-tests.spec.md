...
Harden AI-driver against prompt / shell / trust-boundary injection by landing the following complementary artifacts: (A) a documented **fixture library** of realistic malicious payloads under `tests/injection-fixtures/`, (B) a **static lint workflow** (`.github/workflows/injection-lint.yml`) that mechanically scans command docs and critical workflows for known anti-patterns, (C) a **threat-model document** at `docs/security/injection-threat-model.md` with stable rule-ID anchors, and (D) the standard repo-process updates that accompany any shipped feature (`AGENTS.md` contributor guidance + `CHANGELOG.md` release notes). v0.3.4 introduced the `## Trust boundary` concept and marker-guarded self-ID; v0.3.6 added Phase 0 spec review with data-fence wrapping. But we have no durable test that those guardrails stay in place — a future edit could silently regress them. Fixtures document the attack shape; lint blocks regressions without relying on human review; threat-model doc gives reviewers a stable reference.
...
- [ ] AC-002: every fixture has the required **six** frontmatter keys. `for f in name attack-class target-command mitigation safety-note rule-anchor; do test "$(grep -l "^$f:" tests/injection-fixtures/*.md | wc -l)" -eq 5 || exit 1; done`
...
- [ ] AC-012: **every** lint rule honors the output-format contract. `for r in L-TRUST L-QUOTE L-SELF-ID L-BOT L-EXTRACT; do bash tests/injection-lint-cases/assert-format.sh "$r" >/dev/null 2>&1 || exit 1; done` — each rule's harness run must emit stderr containing `rule-id=<ID>`, a `file:line` reference, `fix:` prefix, and `#<ID>` anchor. Covers MUST-003 across all 5 rules.
...
- [ ] AC-014: both `/ai-driver:run-spec` AND `/ai-driver:review-spec` Pre-flight refuse paths outside `specs/` via a concrete `case` statement that (a) rejects `..` segments and absolute paths, (b) canonicalizes with `pwd -P`, and (c) verifies the resolved path starts with the real `specs/` directory. `for f in plugins/ai-driver/commands/run-spec.md plugins/ai-driver/commands/review-spec.md; do grep -Eq 'case[[:space:]]+"?\$(ARGUMENTS|SPEC_PATH)"?[[:space:]]+in' "$f" && grep -Fq '*..*' "$f" && grep -Fq 'pwd -P' "$f" && grep -Fq 'cd specs' "$f" || exit 1; done`
- [ ] AC-015: no live LLM invocation in **any** script executed by the injection-lint CI path (covers MUSTNOT-002). `for f in .github/workflows/injection-lint.yml .github/scripts/injection-lint.sh tests/injection-lint-cases/run.sh tests/injection-lint-cases/assert-format.sh; do ! grep -nE '^[[:space:]]*(codex[[:space:]]+exec|claude[[:space:]]+(exec|--)|anthropic[[:space:]])' "$f" || exit 1; done`
- [ ] AC-016: no pre-existing guardrail was **deleted** by this branch (covers MUSTNOT-003). Check: for each token that appears in `origin/main`'s copy of a guarded command file, it must still appear in the branch's copy. `for tok in 'Trust boundary' 'SELF_LOGIN' 'user.type' '-s read-only' '--paginate' 'BEGIN SPEC'; do for f in plugins/ai-driver/commands/review-pr.md plugins/ai-driver/commands/fix-issues.md plugins/ai-driver/commands/merge-pr.md plugins/ai-driver/commands/run-spec.md plugins/ai-driver/commands/review-spec.md; do if git show "origin/main:$f" 2>/dev/null | grep -Fq "$tok"; then grep -Fq "$tok" "$f" || { echo "DELETED: $tok was in origin/main:$f but not in branch"; exit 1; }; fi; done; done` — the check only asserts non-deletion of tokens that were already present in `origin/main`, so adding a new guarded command file (like `review-spec.md` in v0.3.6) doesn't trigger a false failure.
- [ ] AC-017: threat model doc has the contracted structure — every lint rule ID has an anchor, every fixture has a named mitigation, and an explicit `## Out of scope` section exists. `test -f docs/security/injection-threat-model.md && for r in L-TRUST L-QUOTE L-SELF-ID L-BOT L-EXTRACT; do grep -Fq "<a id=\"$r\"></a>" docs/security/injection-threat-model.md || exit 1; done && grep -Fq '## Out of scope' docs/security/injection-threat-model.md`
- [ ] AC-018: `.gitattributes` does not mask the fixtures directory (covers MUSTNOT-001). `! test -f .gitattributes || ! grep -E '^tests/injection-fixtures(/|\b).*-(text|diff)|linguist-generated|linguist-vendored' .gitattributes`
