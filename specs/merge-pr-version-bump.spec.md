...
- [ ] AC-009: no null injection on single-entry plugin without `version`: fixture with `{"plugins":[{"name":"x","source":"./"}]}` (no `version`) runs through Step 2 and remains byte-identical (no `"version": null` injected).
- [ ] AC-010: malformed `.plugins` is not an array: fixture with `{"plugins": null}` or with `.plugins` absent entirely runs through Step 2 without aborting mid-way; `marketplace.json` is either left alone (for `metadata.version` absent) or only `metadata.version` is bumped — never produces an error after CHANGELOG has already been rewritten.
- [ ] AC-011: dry-run prints actual manifest diffs: when `.claude-plugin/marketplace.json` exists, dry-run output contains a before/after diff of the manifest (generated by running the jq filter to a tempfile and diffing), not a placeholder. The real `marketplace.json` on disk is byte-identical after dry-run (`cmp` before/after).
