...
- [ ] AC-014: both `/ai-driver:run-spec` AND `/ai-driver:review-spec` Pre-flight refuse paths outside `specs/` via a concrete `case` statement (not just prose). `grep -Eq 'case[[:space:]]+"?\$(ARGUMENTS|SPEC_PATH)"?[[:space:]]+in' plugins/ai-driver/commands/run-spec.md && grep -Eq 'specs/\*' plugins/ai-driver/commands/run-spec.md && grep -Eq 'case[[:space:]]+"?\$(ARGUMENTS|SPEC_PATH)"?[[:space:]]+in' plugins/ai-driver/commands/review-spec.md && grep -Eq 'specs/\*' plugins/ai-driver/commands/review-spec.md`
