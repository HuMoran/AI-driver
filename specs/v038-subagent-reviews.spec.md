...
- [ ] AC-001: `run-spec.md` §Phase 0 Layer 1 invokes a sandboxed executor (subagent), not inline review. Semantic check: Layer 1 section mentions `subagent` AND passes the spec by path (not by interpolated content). `awk '/^### Layer 1:/,/^### Layer 2:/' plugins/ai-driver/commands/run-spec.md | grep -Fq 'subagent' && awk '/^### Layer 1:/,/^### Layer 2:/' plugins/ai-driver/commands/run-spec.md | grep -Fq 'SPEC_PATH'`
- [ ] AC-002: Layer 1 subagent `allowed-tools` is **exactly** `Read, Grep, Glob` (no trailing `Write` / `Bash` / `Agent`). `awk '/^### Layer 1:/,/^### Layer 2:/' plugins/ai-driver/commands/run-spec.md | grep -E '^\s*allowed-tools:' | grep -Eq '^\s*allowed-tools:\s*Read,\s*Grep,\s*Glob\s*$'`
- [ ] AC-003: `review-spec.md` §Layer 1 likewise spawns a subagent whose `allowed-tools` is **exactly** `Read, Grep, Glob`. `awk '/^## Layer 1:/,/^## Layer 2:/' plugins/ai-driver/commands/review-spec.md | grep -Fq 'subagent' && awk '/^## Layer 1:/,/^## Layer 2:/' plugins/ai-driver/commands/review-spec.md | grep -E '^\s*allowed-tools:' | grep -Eq '^\s*allowed-tools:\s*Read,\s*Grep,\s*Glob\s*$'`
...
- [ ] AC-005: Phase 1 plan review invokes BOTH a Claude subagent AND `codex exec` with `-s read-only`, AND the subagent has `allowed-tools: Read, Grep, Glob`. `awk '/^### Plan Review|### Codex Plan Review/,/^## Phase 2:/' plugins/ai-driver/commands/run-spec.md | grep -Fq 'subagent' && awk '/^### Plan Review|### Codex Plan Review/,/^## Phase 2:/' plugins/ai-driver/commands/run-spec.md | grep -Eq 'codex exec[^\n]*-s read-only|-s read-only[^\n]*codex exec' && awk '/^### Plan Review|### Codex Plan Review/,/^## Phase 2:/' plugins/ai-driver/commands/run-spec.md | grep -E '^\s*allowed-tools:' | grep -Eq '^\s*allowed-tools:\s*Read,\s*Grep,\s*Glob\s*$'`
...
- [ ] AC-008: `review-pr.md` Pass 1 spawns a subagent with exact minimal `allowed-tools`. `awk '/^## Step 3|^## Pass 1/,/^## Step 4|^## Pass 2/' plugins/ai-driver/commands/review-pr.md | grep -Fq 'subagent' && awk '/^## Step 3|^## Pass 1/,/^## Step 4|^## Pass 2/' plugins/ai-driver/commands/review-pr.md | grep -E '^\s*allowed-tools:' | grep -Eq '^\s*allowed-tools:\s*Read,\s*Grep,\s*Glob\s*$'`
...
**Forbidden patterns** (caught by AC-011 + AC-012 structural checks):
